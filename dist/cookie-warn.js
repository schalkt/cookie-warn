 ...
 */
(function (fn) {
  "use strict"; // element id for styles

  var elementId = fn + "Box";
  var cookieName = "cookieWarn.accepted"; // get cookieWarn element
  ...
  var attributes = getAttributes(); // set or get cookie

  var cookie = function cookie(name, value, days, path, domain, secure) {
    if (value === undefined) {
      var i,
          x,
          y,
  ...
  window[fn] = {
    accept: function accept() {
      // set the cookie
      cookie(cookieName, true, attributes.expire, attributes.path, attributes.domain, attributes.secure); // remove warning box
    ...
    reject: function reject() {
      ...
      var wbox = document.getElementById(elementId); // show reject information
  ...
  var cookieWarnValue = cookie(cookieName); // check

  var check = function check(warnValue) {
  ...
  var warn = function warn() {
    ...
    var css = {
      style: ["#" + elementId + " {transition:all 0.4s ease-in-out;position:fixed;z-index:999999;bottom:-20px;left:0;right:0;opacity:0;text-align:center;padding:10px;background-color:#212121}", "#" + elementId + " .btn {white-space:nowrap}", "#" + elementId + " .reject_more {padding:0px 10px;display:none;}", "#" + elementId + ".reject .reject_more {display:block;}", "#" + elementId + ".loaded {opacity:0.9;bottom:0px}", "#" + elementId + ".closed {display:none;}"],
      style2: ["#" + elementId + " {font-family: Verdana;line-height:24px;color:#f1f1f1;font-size:14px;}", "#" + elementId + " .btn {text-transform:uppercase;cursor:pointer;background-color:#f1f1f1;color:#659fda;padding:3px 14px;margin-left:10px;}", "#" + elementId + " .btn:hover {background-color:#ffffff;color:#4d78a5;}", "#" + elementId + " a {text-decoration:none;color:#659fda}"],
      type: "text/css",
    ...
    wbox.innerHTML = '<div class="text">' + attributes.data.text + info + accept_button + reject_button + reject_content + "</div>"; // append to body
